...
map_efo <- function(umls_map, efo_release = "v3.29.1", update = T){
  if(!file.exists(paste0("data-raw/efo/efo.",efo_release,".obo")) | 
     update == T){
    download.file(
      paste0("https://github.com/EBISPOT/efo/releases/download/", 
             efo_release,"/efo.obo"), 
      destfile = paste0("data-raw/efo/efo.",efo_release,".obo"))
  }
  con <- file(description = 
                paste0("data-raw/efo/efo.", 
                       efo_release,".obo"), 
              open="r")
  lines <- readLines(con)
  close(con)
  ...
  while(i <= length(lines)){
    ...
    if(stringr::str_detect(line, "id: (EFO|HP|MONDO|GO|Orphanet):[0-9]{1,}$")){
      if(!is.na(efo_id) & 
         !is.na(name) & 
  ...
  cui_map <- efo_map %>%
    ...
    dplyr::left_join(dplyr::filter(umls_map$map, main_term == T), 
                     by = "cui") %>%
  ...
  nci_map <- efo_map %>%
    ...
    dplyr::left_join(dplyr::filter(umls_map$map, 
                                   main_term == T), by = "cui") %>%
    dplyr::filter(!is.na(cui)) %>%
    dplyr::select(efo_id,cui,cui_name) %>%
    dplyr::distinct()
  ...
  return(list("efo2name" = efo2name, "efo2xref" = efo2xref))
}


map_disease_ontology <- function(skip_non_cui_mapped = T, 
                                 release = "2021-03-29", 
                                 umls_map = NULL){
  ...
  if(!file.exists(
    paste0("data-raw/do/TopNodes_Docancerslim.", release_dest,".obo"))){
    download.file(
      url = 
        paste0(do_github_raw_url,"/subsets/TopNodes_DOcancerslim.obo"), 
                  destfile=paste0("data-raw/do/TopNodes_Docancerslim.", 
                                  release_dest,".obo"))
  }
  do_cancer_top <- 
  ...
  if(!file.exists(paste0("data-raw/do/doid.",release_dest,".obo"))){
    download.file(
      url = 
        paste0(do_github_raw_url, "/doid.obo"), 
      destfile=paste0("data-raw/do/doid.", release_dest, ".obo"))
  }
  do_index <- 
    ontologyIndex::get_ontology(
      paste0("data-raw/do/doid.", release_dest, ".obo"), 
      extract_tags = "everything")
  i <- 1
  
  umls_map_lower <- umls_map$map %>%
  ...
  while(i < length(do_ids)){
    ...
    if(stringr::str_detect(do_id,"DOID:")){
      ...
      if(length(all_ancestors) > 0){
        all_ancestor_df <- data.frame("do_id" = all_ancestors, 
                                      stringsAsFactors = F)
        top_node_cancer <- 
          as.data.frame(dplyr::inner_join(all_ancestor_df, 
      ...
      if(length(cui_ids) == 0){
        do_entry <- 
          data.frame("do_id" = do_id, 
                     "do_name" = do_name, 
                     "cui" = NA, "do_cancer_slim" = do_cancer_slim,  
                     stringsAsFactors = F)
        ...
        while(u <= length(cui_ids)){
          do_entry <- 
            data.frame("do_id" = do_id, 
                       "do_name" = do_name, 
                       "cui" = cui_ids[u], "do_cancer_slim" = do_cancer_slim, 
                       stringsAsFactors = F)
          do_map <- rbind(do_map, do_entry)
          u <- u + 1
        }
...
map_umls <- function(update = T){
  if(!file.exists("data-raw/umls/MGCONSO.csv.gz") | update == T){
    download.file("ftp://ftp.ncbi.nlm.nih.gov/pub/medgen/csv/MGCONSO.csv.gz", 
                  destfile = "data-raw/umls/MGCONSO.csv.gz")
  }
  if(!file.exists("data-raw/umls/NAMES.csv.gz") | update == T){
    download.file("ftp://ftp.ncbi.nlm.nih.gov/pub/medgen/csv/NAMES.csv.gz", 
                  destfile = "data-raw/umls/NAMES.csv.gz")
  }
  if(!file.exists("data-raw/umls/MGREL_1.csv.gz") | update == T){
    download.file("ftp://ftp.ncbi.nlm.nih.gov/pub/medgen/csv/MGREL_1.csv.gz", 
                  destfile = "data-raw/umls/MGREL_1.csv.gz")
  }
  if(!file.exists("data-raw/umls/MGREL_2.csv.gz") | update == T){
    download.file("ftp://ftp.ncbi.nlm.nih.gov/pub/medgen/csv/MGREL_2.csv.gz", 
                  destfile = "data-raw/umls/MGREL_2.csv.gz")
  }
  if(!file.exists("data-raw/umls/Neoplasm_Core.txt")){
    download.file(
      "https://evs.nci.nih.gov/ftp1/NCI_Thesaurus/Neoplasm/Neoplasm_Core.txt", 
      destfile = "data-raw/umls/Neoplasm_Core.txt")
  }
  
  
  neoplasm_core <- 
    read.table("data-raw/umls/Neoplasm_Core.txt", sep = "\t", 
               header = T, quote = "", 
               na.strings = c(""), stringsAsFactors = F) %>%
    janitor::clean_names()
  
  umls_rel_1 <- read.csv(gzfile("data-raw/umls/MGREL_1.csv.gz"), 
                         stringsAsFactors = F)
  umls_rel_2 <- read.csv(gzfile("data-raw/umls/MGREL_2.csv.gz"), 
                         stringsAsFactors = F)
  ...
  umls <- read.csv(gzfile("data-raw/umls/MGCONSO.csv.gz"), stringsAsFactors = F)
  
  concept_names_main <- 
    read.csv(gzfile("data-raw/umls/NAMES.csv.gz"), stringsAsFactors = F) %>%
    dplyr::select(CUI,name) %>%
    dplyr::rename(STR = name) %>%
    dplyr::mutate(main_term = TRUE)
  
  concept_summary_data <- 
    read.csv(gzfile("data-raw/umls/MGCONSO.csv.gz"), stringsAsFactors = F) %>%
  ...
  saveRDS(concept_summary_data, 
          file="../data-raw/medgen/concept_summary_data.rds")
  saveRDS(concept_summary_data, 
          file="cui_umls_data.rds")
  ...
  umls <- list("map" = concept_summary_data, 
               "relation" = umls_rel, 
               "nci" = umls_nci, "msh" = umls_msh)
  return(umls)
}

map_oncotree_ontology <- function(umls_map){
  ...
  onco_tree <- mapped_all %>%
    ...
    dplyr::filter(cui != "C0346202" | cui == "C0346202" & group == "Cervical Cancer, NOS") %>%
    dplyr::filter(cui != "C1332912" | cui == "C1332912" & group == "Cervical Cancer, NOS") %>%
    dplyr::filter(cui != "C0024299" | cui == "C0024299" & group == "Lymphatic Cancer, NOS") %>%
    dplyr::distinct()
  
  syndromes_ding <- openxlsx::read.xlsx("data-raw/hereditary_cancer/ding_pancancer_cell_2018.xlsx",sheet = 4,startRow = 1) %>%
  ...
  susceptibility_ding <- openxlsx::read.xlsx("data-raw/hereditary_cancer/ding_pancancer_cell_2018.xlsx",sheet = 4,startRow = 1) %>%
    ...
    dplyr::mutate(cui = stringr::str_trim(cui), name = "Hereditary Cancer Susceptibility, NOS") %>%
    dplyr::filter(cui != "C0238198" & cui != "C0027819" & cui != "C1333989") %>%
    dplyr::distinct()
  
  hereditary_cancers <- openxlsx::read.xlsx("data-raw/hereditary_cancer/tumor_nos_umls.xlsx",sheet = 1) %>%
